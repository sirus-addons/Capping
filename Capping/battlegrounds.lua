...
---------------------------
	if not self.WSGBulk then  -- init some data and create carrier frames
		...
		local function SetWSGCarrierAttribute()
			...
			af:SetPoint("LEFT", UIParent, "BOTTOMLEFT", WorldStateTopCenterFrameLeftBar:GetLeft() - 38, WorldStateTopCenterFrameLeftBar:GetBottom() - 32)
			hf:SetAlpha(1)
			hf:SetAttribute("macrotext", format("/target %s", hcarrier or ""))
			hf:SetPoint("LEFT", UIParent, "BOTTOMLEFT", WorldStateTopCenterFrameRightBar:GetLeft(), WorldStateTopCenterFrameRightBar:GetBottom() - 32)
		end
		...
		local ovverideFactionID = C_Unit:GetFactionByDebuff("player")
		if ovverideFactionID then
			playerfaction = ovverideFactionID == PLAYER_FACTION_GROUP.Alliance and "Alliance" or "Horde";
		end
